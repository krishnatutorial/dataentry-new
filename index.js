...
const dbPath2 = path.join(__dirname, 'dataentry.db')
...
const verifyToken = (req, res, next) => {
  try {
    ...
    console.log(
      'Error occured in verifying token, please send jwt token for authentication',
      e,
    )
  }
}
...
//DataEntry related api's

//creating DataEntry
app.post('/newDataEntry', verifyToken, async (req, res) => {
  try {
    await DataEntry.addDataEntry(req, res, db2)
  } catch (e) {
    console.log('Failed to add new DataEntry', e)
  }
})

//Retrieving DataEntry
app.get('/data/:id', verifyToken, async (req, res) => {
  try {
    await DataEntry.getDataEntry(req, res, db2)
  } catch (e) {
    console.log('Failed to get the DataEntry with the given id', e)
  }
})

//Update DataEntry

app.put('/dataUpdate/:id', verifyToken, async (req, res) => {
  try {
    await DataEntry.updateDataEntry(req, res, db2)
  } catch (e) {
...
//Delete DataEntry
app.delete('/dataDelete/:id', verifyToken, async (req, res) => {
  try {
    await DataEntry.DeleteDataEntry(req, res, db2)
  } catch (e) {
